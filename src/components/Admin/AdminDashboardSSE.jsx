...
function AdminDashboardSSE() {
    ...
    return (
        <Box sx={{ padding: 2 }}>
            ...
            <Stack spacing={2}>
                <Typography>
                    {t("totalUsersAdmin")} {dashboardData.totalUsers}
                </Typography>

                <Typography>
                    {t("totalAttemptsAdmin")} {dashboardData.totalAttempts}
                </Typography>
                ...
                <Box>
                    <Typography variant="subtitle1">
                        {t("attemptsByTopicAdmin")}
                    </Typography>

                    <Table>
                        <TableBody>
                            {Object.entries(dashboardData.attemptsByTopic).map(
                                ([topic, count]) => (
                                    <TableRow key={topic}>
                ...
                <Box>
                    <Typography variant="subtitle1">
                        {t("successRateByTopicAdmin")}
                    </Typography>

                    <Table>
                        <TableBody>
                            {Object.entries(dashboardData.successRateByTopic).map(
                                ([topic, rate]) => (
                                    <TableRow key={topic}>
                                        <TableCell>{t(topic)}</TableCell>
